...
def main():
    ...
    parser.add_argument('--idpentryurl',
        help='Entry URL into the web auth for SAML for AWS.'
        ' Defaults to https://aws.cloud.upenn.edu.'
    )
    ...
    log_level = 'INFO'
    if args.logging:
        log_level = logging.getLevelName(args.logging.upper())
    elif os.getenv("DEBUG", None):
    ...
    if not os.path.exists(awsconfigfile): 
        ...
        with open(expanduser(awsconfigfile), 'w'): pass 
    ...
    if args.idpentryurl:
        ...
        idpentryurl = os.getenv("IDPURL", 'http://aws.cloud.upenn.edu')
